# -*- coding: utf-8 -*-
from pygameframework import ConfigFile
from pygameframework import GameRunner
...
class KeyConfig(Game):
    def __init__(self, controller_num):
        Game.__init__(self)
        config = ConfigFile('config.ini').load()
    ...
    def _load_key_list(self, config):
        keys_setting = config.items('Keys')
        if not keys_setting: return
        [(rag, key_list)] = keys_setting
...
if __name__ == '__main__':
    CONTROLLER_NUM = 4
    GameRunner(KeyConfig(CONTROLLER_NUM))\
        .initialize_system()\
